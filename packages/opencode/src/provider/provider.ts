...
export namespace Provider {
  ...
  const CUSTOM_LOADERS: Record<string, CustomLoader> = {
    ...
    async altare() {
      return {
        autoload: true,
        options: {
          // Altare Arc API does not require an API key
  ...
  export async function getSmallModel(providerID: string) {
    ...
    if (providerID === "altare") {
      priority = ["arc-4-mini-2511"]
    }
  ...
  const priority = ["arc-4-2511", "arc-4-max-2511", "gemini-2.5-pro-preview", "gpt-5", "claude-sonnet-4"]
